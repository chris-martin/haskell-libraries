...
-- | A 'Span' consists of a start location ('start') and an end location
-- ('end'). The end location must be greater than the start location; in
-- other words, empty or backwards spans are not permitted.
--
-- Construct and combine 'Span's using 'fromTo', '+', and '-'.
data Span = Span
  { start :: Loc
  , end   :: Loc
  } deriving (Eq, Ord)
...
-- |
-- >>> fromTo (read "3:14") (read "6:5")
-- 3:14-6:5
instance Show Span
  where

    showsPrec _ (Span a b) =
      showsPrec 10 a .
      (showString "-") .
      showsPrec 10 b
...
-- | Attempt to construct a 'Span' from two 'Loc's. The lesser 'Loc' will be
-- the start, and the greater 'Loc' will be the end. The two 'Loc's must not
-- be equal, or else this throws 'EmptySpan'.
--
...
-- | Attempt to construct a 'Span' from two 'Loc's. The lesser 'Loc' will be
-- the start, and the greater 'Loc' will be the end. If the two 'Loc's are not
-- equal, the result is 'Nothing', because a 'Span' cannot be empty.
--
