...
-- /position/, because we use it to represent line and column numbers
-- ('Line' and 'Column').
--
...
instance Show Pos
  where
    ...
    showsPrec i (Pos n) = showsPrec i n
...
instance Read Pos
  where
    ...
    readPrec = Pos <$> mfilter (/= 0) readPrec
...
-- normally call for 'toInteger' (which we cannot use because 'Pos' does not
-- have an instance of 'Integral').
class ToNat a
  where
